...
def create_signal(tR, W, tlim):
    ...
    x = np.linspace(0, tlim, 1000)
    signal = np.zeros(1000)
    for i in range(len(tR)):
        signal += norm.pdf(x, loc=tR[i], scale=sigmas[i])
    return x, signal

def detect_peaks(x, y, height_thresh=0, plot=False, peak_info=False):
    """
    ...
    :param peak_info: Bool, If true, will also return the peak height, and peak valleys
    Returns
    ----------
    ...
    if plot == True:
        ...
        ax.plot(xvals[right_bases], y[right_bases], ".")
        # ax.plot(xvals[left_bases], y[left_bases], ".")
        ...
        plt.show()
    if peak_info:
        return x[peaks], widths * dt, y[peaks], x[right_bases], y[right_bases]
    else:
        return x[peaks], widths * dt


# To testing_brute.py the code
